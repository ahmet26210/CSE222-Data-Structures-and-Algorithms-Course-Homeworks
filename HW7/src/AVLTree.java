...
public class AVLTree < E
  ...
  public AVLTree<E> headSet(E e) {
		...
		AVLTree<E> headSet = new AVLTree<E>();
		headSet(root,e,headSet);
		return headSet;
	}

	
	private AVLTree<E> headSet(Node<E> local,E item,AVLTree<E> headSet){
		if(local == null)
			return headSet;
			
		headSet(local.left,item,headSet);
		if(item.compareTo(local.data) > 0)
			headSet.add(local.data);
	...
	public AVLTree<E> tailSet(E e) {
		AVLTree<E> tailSet = new AVLTree<E>();
		tailSet(root,e,tailSet);
		return tailSet;
	}
	
	
	private AVLTree<E> tailSet(Node<E> local,E item,AVLTree<E> tailSet){
		if(local == null)
			return tailSet;
			
		tailSet(local.left,item,tailSet);
		if(item.compareTo(local.data) < 0)
			tailSet.add(local.data);
